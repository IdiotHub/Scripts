					...
					task.spawn(function()
                    	...
                    	local function Set(Value: number)
                    		if Value then
                    			...
                    			Element.Instance.Value.input.CursorPosition = #Element.Instance.Value.input.Text + 1
                    	...
                    	UserInputService.InputChanged:Connect(function(input)
                    		if not Element.SLDragging then return end
                    		...
                    		local bar = Element.Instance.PART_Backdrop
                    		...
                    		local value =
                    			(Element.Values.Range[2] - Element.Values.Range[1]) * percent
                    			+ Element.Values.Range[1]
                    
                    		value =
                    			math.floor(value / Element.Values.Increment + 0.5)
                    			* Element.Values.Increment
                    
                    		if value ~= Element.Values.CurrentValue then
                    			Set(value)
                    		end
                    	end)
                    	...
                    	input:GetPropertyChangedSignal("Text"):Connect(function()
                    		...
                    		local sanitized = newText:gsub("[^%d%.]", "")
                    		if sanitized ~= newText then
                    			updating = true
                    			tb.Text = sanitized
                    			tb.CursorPosition = #sanitized + 1
                    			updating = false
                    		end
